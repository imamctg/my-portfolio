...
export default function WithdrawPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      const [statsRes, earningsRes] = await Promise.all([
        fetch(`${baseURL}/affiliate/stats`, {
          headers: { Authorization: `Bearer ${token}` },
          signal: controller.signal,
        }),
        fetch(`${baseURL}/affiliate/earnings`, {
          headers: { Authorization: `Bearer ${token}` },
          signal: controller.signal,
        }),
      ])
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const check = await fetch(`${baseURL}/affiliate/stats`, {
        headers: { Authorization: `Bearer ${token}` },
      })

      if (!check.ok) throw new Error('Failed to verify balance')

      const { totalEarnings, totalWithdrawn } = await check.json()
      const latestAvailable = (totalEarnings || 0) - (totalWithdrawn || 0)

      if (amountNum > latestAvailable) {
        throw new Error(
          `Insufficient balance. Available: ${CURRENCY(latestAvailable)}`
        )
      }

      const res = await fetch(`${baseURL}/withdraw/request`, {
        method: 'POST',
        headers: {
        ...
        <Card>
          <CardContent className='p-4'>
            ...
            <div className='mt-2 text-xs text-muted-foreground flex gap-4'>
              <span>Today: {CURRENCY(earnings.today)}</span>
              <span>Month: {CURRENCY(earnings.monthly)}</span>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent>
            <div className='grid gap-4 md:grid-cols-3'>
              ...
              <div className='space-y-2'>
                ...
                {amountNum > 0 && amountNum < MIN_WITHDRAWAL_AMOUNT && (
              ...
              <div className='space-y-2'>
                ...
                <Input
                  type='text'
                  placeholder={
                    method ? METHOD_PLACEHOLDER[method] : t('Account Details')
                  }
                ...
                />
                <p className='text-xs text-muted-foreground'>
                  Make sure the account is active & correct.
                </p>
              </div>
            </div>
